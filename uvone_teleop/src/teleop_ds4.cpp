...
struct LightControl {
    ...
    void update(const ds4_driver::Status::ConstPtr& msg)
    {
        ...
        if (msg->button_dpad_left)
            pub_msg.lamp_selected = uvone_msgs::LightCmd::SELECT_LEFT_LAMP;

        if (msg->button_dpad_right)
            pub_msg.lamp_selected = uvone_msgs::LightCmd::SELECT_RIGHT_LAMP;

        if (msg->button_options)
            pub_msg.lamp_selected = uvone_msgs::LightCmd::SELECT_NONE_LAMP;
