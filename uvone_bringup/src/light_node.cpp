...
public:
    ...
    LampNode_t(ros::NodeHandle& nh)
     :  digital_node( nh )
    {
        while(!digital_node.has_connection() && ros::ok())
...
struct LightNode_t {
    ...
    void callback(const uvone_msgs::LightCmd::ConstPtr& msg)
    {
        if (msg->inverter != 0)
        {
            if (door_status)
        ...
        {
            case uvone_msgs::LightCmd::SELECT_NONE_LAMP:
                lamp_node.set_lamp(LampNode_t::SelectorLamp::NONE);
                break;
            case uvone_msgs::LightCmd::SELECT_LEFT_LAMP:
                lamp_node.set_lamp(LampNode_t::SelectorLamp::LEFT_LAMP);
                break;
            case uvone_msgs::LightCmd::SELECT_RIGHT_LAMP:
